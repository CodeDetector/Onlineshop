...
class SignIn extends React.Component{
    ...
    handleSubmit=async (event)=>{
        ...
        try{
            ...
            // firebase.firestore().collection("users").doc(this.state.currentUserEmail).collection("follows").doc(email).get() .then((docSnapshot) => { var check; if (docSnapshot.exists){ Alert.alert("hi") return Promise.reject( true ); }else{ Alert.alert("not hi") return Promise.reject( false ); } return (check.promise) })
        }catch(error){
            console.log(error);
        }
